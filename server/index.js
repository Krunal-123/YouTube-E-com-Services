...
async function main(){
    let url="mongodb://127.0.0.1:27017/youtube"
            ...
            bcrypt.compare(password,User.password).then((result,err)=>{
                if (result) {
                    if (remember=="remember") {
                        ...
                        return res.cookie('token',token,{ maxAge: 28 * 24 * 60 * 60 * 1000 }).send('ok')
                    }
                    else{
                        let token=jwt.sign(email,process.env.JWT_SECRET)
...
.post("/addcart/user",async(req,res)=>{
    try {
        ...
        let decode= jwt.verify(cookies.token,process.env.JWT_SECRET)
        let User=await user.find({email:decode}).populate("addcart").populate("myitems").populate("myfavourites").populate('orderhistory.id')
        res.send(User)
    } catch (error) {
        console.log(error);   
    }
