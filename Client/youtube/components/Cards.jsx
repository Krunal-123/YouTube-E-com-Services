...
export default function ({ id, img, title, description, btn, email, status,color }) {
  ...
  return (
    <Card className='border-0 w-[350px] h-[500px] m-auto transition-transform duration-300 hover:scale-105 shadow-2xl hover:shadow-[0_0px_40px_1px_rgba(0,0,0,1)] relative rounded'>
      ...
      <Link to={`/services/${id}`}>
        {<Badge bg={color} style={{ position: 'absolute', top: '-10px', left: '15px' }}>{status}</Badge>}
        ...
        <Card.Body className='p-4'>
          ...
          <Card.Text className='text-gray-700'>{description && description.slice(0, 150)+'...'}<span className='text-blue-600 font-bold hover:underline'>(More)</span></Card.Text>
